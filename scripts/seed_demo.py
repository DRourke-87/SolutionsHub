    ...
    with get_sessionmaker()() as db:
        ...
        for d in DEMO:
            ...
            sub = Submission(
                ...
                key_differentiators=d["differentiators"],
                readiness_level=d["readiness"],
                readiness_programs="Demonstration data",
                on_proposal=d["on_proposal"],
                current_pipeline="Demonstration data",
                resource_links_notes="https://www.amentum.com/",
                relevant_partnerships="Demonstration data",
                sensitive_data_ack_at=when - timedelta(days=21),
                sensitive_data_ack_by_email=d["owner"][1],
            ...
            sub.contacts.append(
                SubmissionContact(
                    contact_role=ContactRole.OWNER_TECHNICAL.value, name=d["owner"][0], email=d["owner"][1]
                )
            )
            for code in d["caps"]:
                if code in caps:
